...
import com.siri.travelguidemultiswipedbrealtime.util.SwipeItemDecoration
...
open class SwipeRecyclerView @JvmOverloads constructor(
    ...
    enum class ListOrientation(
        internal var flagsDragVal: Int,
        internal var flagsswipeVal : Int
    ) {
        ...
        /** Direction flags for the drag action. Determines in which direction the user gestures will cause dragging to happen. **/
        val dragDirectionFlags: List<DirectionFlag>
            get() {
                val dragFlagsList = mutableListOf<DirectionFlag>()
                if (flagsDragVal and DirectionFlag.UP.value == DirectionFlag.UP.value)
                    dragFlagsList.add(DirectionFlag.UP)
                if (flagsDragVal and DirectionFlag.DOWN.value == DirectionFlag.DOWN.value)
                    dragFlagsList.add(DirectionFlag.DOWN)
                if (flagsDragVal and DirectionFlag.LEFT.value == DirectionFlag.LEFT.value)
                    dragFlagsList.add(DirectionFlag.LEFT)
                if (flagsDragVal and DirectionFlag.RIGHT.value == DirectionFlag.RIGHT.value)
                    dragFlagsList.add(DirectionFlag.RIGHT)

                return dragFlagsList
            }

        /** Direction flags for the swipe action. Determines in which direction the user gestures will cause swiping to happen. **/
        val swipeDirectionFlags: List<DirectionFlag>
            get() {
                val swipeFlagsList = mutableListOf<DirectionFlag>()
                if (flagsswipeVal and DirectionFlag.UP.value == DirectionFlag.UP.value)
                    swipeFlagsList.add(DirectionFlag.UP)
                if (flagsswipeVal and DirectionFlag.DOWN.value == DirectionFlag.DOWN.value)
                    swipeFlagsList.add(DirectionFlag.DOWN)
                if (flagsswipeVal and DirectionFlag.LEFT.value == DirectionFlag.LEFT.value)
                    swipeFlagsList.add(DirectionFlag.LEFT)
                if (flagsswipeVal and DirectionFlag.RIGHT.value == DirectionFlag.RIGHT.value)
     ...
     *
     * Please note that, for  horizontal lists need a divider drawable with a defined width,
    ...
    override fun onSaveInstanceState(): Parcelable? {
        ...
        if (isSaveEnabled) {
            val bundle = Bundle()
            bundle.putParcelable(DATA_SUPER_STATE, superState)
            bundle.putInt(DATA_ITEM_LAYOUT_ID, itemLayoutId)
            bundle.putInt(DATA_DIVIDER_SPLIT_ID, dividerDrawableId ?: 0)
            bundle.putInt(
                DATA_BEHIND_SWIPED_ITEM_ICON_DRAWABLE_ID,
                behindSwipedItemIconDrawableId ?: 0
            )
            bundle.putInt(
                DATA_DATA_BEHIND_SWIPED_ITEM_ICON_SECONDARY_DRAWABLE_ID,
                behindSwipedItemIconSecondaryDrawableId ?: 0
            )
            bundle.putFloat(DATA_BEHIND_SWIPED_ITEM_ICON_MARGIN, behindSwipedItemIconMargin)
            bundle.putBoolean(DATA_BEHIND_SWIPED_ITEM_CENTER_ICON, behindSwipedItemCenterIcon)
            bundle.putInt(
                DATA_BEHIND_SWIPED_ITEM_BACKGROUND_COLOR,
                behindSwipedItemBackgroundColor ?: Color.TRANSPARENT
            )
            bundle.putInt(
                DATA_BEHIND_SWIPED_ITEM_BACKGROUND_SECONDARY_COLOR,
                behindSwipedItemBackgroundSecondaryColor ?: Color.TRANSPARENT
            )
            bundle.putInt(DATA_BEHIND_SWIPED_ITEM_LAYOUT_ID, behindSwipedItemLayoutId ?: 0)
            bundle.putInt(
                DATA_BEHIND_SWIPED_ITEM_SECONDARY_LAYOUT_ID,
                behindSwipedItemSecondaryLayoutId ?: 0
            )
            bundle.putBoolean(DATA_REDUCE_ITEM_ALPHA_ON_SWIPING, reduceItemAlphaOnSwiping)
            bundle.putBoolean(DATA_LONG_PRESS_TO_START_DRAGGING, longPressToStartDragging)
            bundle.putInt(DATA_NUM_OF_COLUMNS_PER_ROW_IN_GRID_LIST, numOfColumnsPerRowInGridList)
            bundle.putInt(DATA_NUM_OF_ROWS_PER_COLUMN_IN_GRID_LIST, numOfRowsPerColumnInGridList)
            bundle.putString(DATA_ORIENTATION_NAME, orientation?.name)
            bundle.putInt(DATA_DISABLED_DRAG_FLAGS_VALUE, disabledDragFlagsValue)
            bundle.putInt(DATA_DISABLED_SWIPE_FLAGS_VALUE, disabledSwipeFlagsValue)
    ...
    override fun onRestoreInstanceState(state: Parcelable?) {
        ...
        if (isSaveEnabled && state is Bundle) {
            superState = state.getParcelable(DATA_SUPER_STATE)
            itemLayoutId = state.getInt(DATA_ITEM_LAYOUT_ID, 0)
            dividerDrawableId = state.getInt(DATA_DIVIDER_SPLIT_ID, 0)
            behindSwipedItemIconDrawableId = state.getInt(DATA_BEHIND_SWIPED_ITEM_ICON_DRAWABLE_ID, 0)
            behindSwipedItemIconSecondaryDrawableId = state.getInt(
                DATA_DATA_BEHIND_SWIPED_ITEM_ICON_SECONDARY_DRAWABLE_ID, 0)
            behindSwipedItemIconMargin = state.getFloat(DATA_BEHIND_SWIPED_ITEM_ICON_MARGIN, 0f)
            behindSwipedItemCenterIcon = state.getBoolean(DATA_BEHIND_SWIPED_ITEM_CENTER_ICON, false)
            behindSwipedItemBackgroundColor = state.getInt(DATA_BEHIND_SWIPED_ITEM_BACKGROUND_COLOR, Color.TRANSPARENT)
            behindSwipedItemBackgroundSecondaryColor = state.getInt(
                DATA_BEHIND_SWIPED_ITEM_BACKGROUND_SECONDARY_COLOR, Color.TRANSPARENT)
            behindSwipedItemLayoutId = state.getInt(DATA_BEHIND_SWIPED_ITEM_LAYOUT_ID, 0)
            behindSwipedItemSecondaryLayoutId = state.getInt(
                DATA_BEHIND_SWIPED_ITEM_SECONDARY_LAYOUT_ID, 0)
            reduceItemAlphaOnSwiping = state.getBoolean(DATA_REDUCE_ITEM_ALPHA_ON_SWIPING, false)
            longPressToStartDragging = state.getBoolean(DATA_LONG_PRESS_TO_START_DRAGGING, false)
            numOfColumnsPerRowInGridList = state.getInt(DATA_NUM_OF_COLUMNS_PER_ROW_IN_GRID_LIST, 1)
            numOfRowsPerColumnInGridList = state.getInt(DATA_NUM_OF_ROWS_PER_COLUMN_IN_GRID_LIST, 1)
            val savedOrientationName = state.getString(DATA_ORIENTATION_NAME, null)
            if (savedOrientationName != null && savedOrientationName.isNotEmpty())
                orientation = ListOrientation.valueOf(savedOrientationName)
            disabledDragFlagsValue = state.getInt(DATA_DISABLED_DRAG_FLAGS_VALUE, 0)
            disabledSwipeFlagsValue = state.getInt(DATA_DISABLED_SWIPE_FLAGS_VALUE, 0)
        }
    ...
    companion object {
        const val DATA_SUPER_STATE= "super_state"
        const val DATA_ITEM_LAYOUT_ID = "item_layout_id"
        const val DATA_DIVIDER_SPLIT_ID = "divider_drawable_id"
        const val DATA_BEHIND_SWIPED_ITEM_ICON_DRAWABLE_ID = "behind_swiped_item_icon_drawable_id"
        const val DATA_DATA_BEHIND_SWIPED_ITEM_ICON_SECONDARY_DRAWABLE_ID = "behind_swiped_item_icon_secondary_drawable_id"
        const val DATA_BEHIND_SWIPED_ITEM_ICON_MARGIN = "behind_swiped_item_icon_margin"
        const val DATA_BEHIND_SWIPED_ITEM_CENTER_ICON = "behind_swiped_item_center_icon"
        const val DATA_BEHIND_SWIPED_ITEM_BACKGROUND_COLOR = "behind_swiped_item_background_color"
        const val DATA_BEHIND_SWIPED_ITEM_BACKGROUND_SECONDARY_COLOR = "behind_swiped_item_background_secondary_color"
        const val DATA_BEHIND_SWIPED_ITEM_LAYOUT_ID = "behind_swiped_item_layout_id"
        const val DATA_BEHIND_SWIPED_ITEM_SECONDARY_LAYOUT_ID = "behind_swiped_item_secondary_layout_id"
        const val DATA_REDUCE_ITEM_ALPHA_ON_SWIPING = "reduce_item_alpha_on_swiping"
        const val DATA_LONG_PRESS_TO_START_DRAGGING = "long_press_to_start_dragging"
        const val DATA_NUM_OF_COLUMNS_PER_ROW_IN_GRID_LIST = "num_of_columns_per_row_in_grid_list"
        const val DATA_NUM_OF_ROWS_PER_COLUMN_IN_GRID_LIST = "num_of_rows_per_column_in_grid_list"
        const val DATA_ORIENTATION_NAME = "orientation_name"
        const val DATA_DISABLED_DRAG_FLAGS_VALUE = "disabled_drag_flags_value"
        const val DATA_DISABLED_SWIPE_FLAGS_VALUE = "disabled_swipe_flags_value"
    }
}